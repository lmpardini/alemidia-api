...
class ImportarBuffetCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'importar:buffet';
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $buffets = (new BuffetImport())->toArray('storage/buffet.xlsx');
            ...
            foreach ($buffets as $buffet) {
                foreach ($buffet as $index => $cl) {
                    ...
                    $verificaCadastro = Buffet::where('cpf_cnpj', $cpf)->first();
                    ...
                    $novoCadastroBuffet = new Buffet();
                    $novoCadastroBuffet->id = $cl['id'];
                    $novoCadastroBuffet->tipo_cadastro = $tipoCadastro;
                    $novoCadastroBuffet->nome_razao_social = ucwords(strtolower(trim($cl['no'])));
                    $novoCadastroBuffet->slug = Str::slug(ucwords(strtolower(trim($cl['no']))), '_');
                    $novoCadastroBuffet->mail = trim($cl['ml']);
                    $novoCadastroBuffet->cpf_cnpj = $cpf;
                    $novoCadastroBuffet->telefone = $foneFixo;
                    $novoCadastroBuffet->celular = $cel1;                   2;
                    $novoCadastroBuffet->cep = str_replace('-', '', $cl['en_cp']);
                    $novoCadastroBuffet->logradouro = ucwords(strtolower(trim($cl['en'])));
                    $novoCadastroBuffet->numero = trim($cl['en_nu']);
                    $novoCadastroBuffet->complemento = trim($cl['en_cm']);
                    $novoCadastroBuffet->bairro = ucwords(strtolower(trim($cl['en_br'])));
                    $novoCadastroBuffet->cidade = ucwords(strtolower(trim($cl['en_ci'])));
                    $novoCadastroBuffet->estado = $estado;
                    $novoCadastroBuffet->ativo = $cl['at'];
                    $novoCadastroBuffet->save();

                    $this->info('Importação Buffet '. $index+1 .' concluida');
